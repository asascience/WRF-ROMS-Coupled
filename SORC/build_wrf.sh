#!/bin/csh -fx
#
# svn $Id: build_wrf.sh 117 2019-04-30 21:15:40Z arango $
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
#--------------------------------------------------------------------------
 ...
 setenv ROMS_SRC_DIR         $TOPDIR/ROMS
 setenv WRF_ROOT_DIR         $TOPDIR/WRF
...
#setenv which_MPI            openmpi     # compile with OpenMPI library
 setenv which_MPI            impi        # compile with IntelMPI library
...
#setenv USE_REAL_DOUBLE      on          # use real double precision (-r8)
 ...
 setenv HDF5 /usrx/hdf5/1.10.5
 setenv JASPERLIB /usr/lib64
 setenv JASPERINC /usr/include/jasper
#--------------------------------------------------------------------------
...
echo "${separator}"
echo ""
echo "  ln -sfv ${WRF_ROOT_DIR}/run/namelist.input ${WRF_BUILD_DIR}"
...
if ( $move == 1 ) then
  ...
  cp -p test/em_real/README.namelist ${WRF_BUILD_DIR}
  cp -p run/namelist.input ${WRF_BUILD_DIR}
  cp -p frame/md_calls.inc ${WRF_BUILD_DIR}
  cp -p frame/module_dm.F ${WRF_BUILD_DIR}
  cp -p frame/module_state_description.F ${WRF_BUILD_DIR}
  cp -p external/io_grib1/io_grib1.f90 ${WRF_BUILD_DIR}
  cp -p external/io_grib_share/io_grib_share.f90 ${WRF_BUILD_DIR}

  cp -p tools/gen_comms.c ${WRF_BUILD_DIR}

  cp -p inc/dm_comm_cpp_flags ${WRF_BUILD_DIR}
  cp -p inc/wrf_io_flags.h ${WRF_BUILD_DIR}
  cp -p inc/wrf_status_codes.h ${WRF_BUILD_DIR}

  cp -p external/io_int/io_int_idx_tags.h ${WRF_BUILD_DIR}
...
#    if ( -d $DIR ) then
#      cp -p ${DIR}/*.f90 ${WRF_BUILD_DIR}
#      cp -p ${DIR}/*.F90 ${WRF_BUILD_DIR}
#      cp -p ${DIR}/*.inc ${WRF_BUILD_DIR}
#    endif
#  end

  find ${WRF_ROOT_DIR} -type f -name "*.mod" -exec cp -p {} ${WRF_BUILD_DIR} \;
  find ${WRF_ROOT_DIR} -type f -name "*.o"   -exec cp -p {} ${WRF_BUILD_DIR} \;
  find ${WRF_ROOT_DIR} -type f -name "*.obj" -exec cp -p {} ${WRF_BUILD_DIR} \;
  find ${WRF_ROOT_DIR} -type f -name "*.a"   -exec cp -p {} ${WRF_BUILD_DIR} \;

  find ${WRF_ROOT_DIR} -type f -name "*.exe" -exec cp -p {} ${WRF_BIN_DIR} \;
  find ${WRF_ROOT_DIR} -type l -name "*.exe" -exec /bin/rm -fv {} \;

  cp -p external/esmf_time_f90/*.f ${WRF_BUILD_DIR}
  cp -p external/esmf_time_f90/MYESMF*.inc ${WRF_BUILD_DIR}

  cp -p external/io_int/diffwrf ${WRF_BIN_DIR}/diffwrf_int
  cp -p external/io_int/test_io_idx ${WRF_BIN_DIR}
  cp -p external/io_netcdf/diffwrf ${WRF_BIN_DIR}/diffwrf_nc

  cp -p tools/fseeko_test ${WRF_BIN_DIR}
  cp -p tools/registry ${WRF_BIN_DIR}
endif
...
if ( $WRF_CASE == "em_real" ) then
  ...
  mkdir -p ${MY_PROJECT_DIR}/data
  cd ${MY_PROJECT_DIR}/data
...
mv $WRF_BIN_DIR/wrf.exe $EXECDIR
mv $WRF_BIN_DIR/real.exe $EXECDIR
